...
void bench_add_random_single_thread(const int bound) {
  ...
  std::cout << "bench_add_random_single_thread for total " << bound/(1<<20) << " million, duration(us) = " << duration.count() << '\n';
}
...
void bench_add_random_multi_threads(const int thread_num, const int bound) {
  ...
  std::cout << ", duration(us) = " << duration.count() << '\n';
}
...
void bench_add_random_by_one_skipset(const int bound) {
  ...
  const auto end_time = std::chrono::steady_clock::now();
  const auto duration = std::chrono::duration_cast<std::chrono::microseconds>(end_time - start_time);
  std::cout << "bench_add_random_by_skipset for total " << bound/(1<<20) << " million, duration (us) = " << duration.count() << '\n';
}

void bench_add_random_by_vector_skip_set(const int bound) {
  const auto nums = random_nums(bound);
  ...
  const auto end_time = std::chrono::steady_clock::now();
  const auto duration = std::chrono::duration_cast<std::chrono::microseconds>(end_time - start_time);
  std::cout << "bench_add_random_by_vector_skip_set for total " << bound/(1<<20) << " million, duration (us) = " << duration.count() << '\n';
}
...
void bench_add_random_by_multi_skipset(const int ss_num, const int bound) {
  ...
  const auto duration = std::chrono::duration_cast<std::chrono::microseconds>(std::chrono::steady_clock::now() - start_time);
  std::cout << "bench_add_random_by_multi_skipset, " << ss_num << " threads each with one SkipSet, duration (us) = " << duration.count() << '\n';
}
...
void bench_add() {
  ...
  // bench_add_random_by_vector_skip_set(8<<20);
}
